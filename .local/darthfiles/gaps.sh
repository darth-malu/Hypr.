    ...
    case $1 in
        "gaps_in_add")
            ...
            then
                ...
                dunstify -t 1000 -a "changegaps" -u low -i ~/.local/darthfiles/iconss/up.png -h string:x-dunst-stack-tag:$msgTag "Gaps in: $new_gap" 
            fi
            ;;
        "gaps_in_sub")
            ...
            then
                local new_gap=$(( $current_gap_in - $counter_gaps_in ))
                hyprctl keyword general:gaps_in ${new_gap}
                dunstify -t 1000 -a "changegaps" -u low -i ~/.local/darthfiles/iconss/down3.png -h string:x-dunst-stack-tag:$msgTag "Gaps in: $new_gap" 
            fi
            ;;
        "gaps_in_reset")
            hyprctl keyword general:gaps_in $default_gaps_in
            dunstify -t 1000 -a "changegaps" -u low -i ~/.local/darthfiles/iconss/down3.png -h string:x-dunst-stack-tag:$msgTag "Gaps reset to default $default_gaps_in" 
            ;;
        "increment_gap")
