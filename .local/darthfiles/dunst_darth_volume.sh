...
function get_volume () {
    ...
    #volume="$(wpctl get-volume @DEFAULT_SINK@ | awk '{print $2}'| awk -F'.' '{print $2}')" 
...
function sink_switcher (){
    ...
    if [[ $1 == "speaker" ]];then
        ...
        dunstify -t 1000 -a "changeVolume" -u low -i ~/.local/darthfiles/iconss/speaker_94.png \
            -h string:x-dunst-stack-tag:$msg "         Speakers" 
    elif [[ $1 == "earphones" ]];then
        wpctl set-default $earphones_ID
        dunstify -t 1000 -a "changeVolume" -u low -i ~/.local/darthfiles/iconss/earp.png \
            -h string:x-dunst-stack-tag:$msg "        Earphones" 
    elif [[ $1 == "easy" ]];then
        wpctl set-default $easy_sink_ID
        dunstify -t 1000 -a "changeVolume" -u low -i ~/.local/darthfiles/iconss/ez.png \
    ...
    case $1 in
        "add_sub")
            if [[ $int_volume -eq $zero ]];then
                dunstify -t 1000 -a "changeVolume" -u low -i ~/.local/darthfiles/iconss/no_s.png \
                    -h string:x-dunst-stack-tag:$msgTag "Volume Zero" 
            elif [[ $int_volume -eq $max ]];then
                dunstify -t 1000 -a "changeVolume" -u low -i ~/.local/darthfiles/iconss/audio1.png \
                    -h string:x-dunst-stack-tag:$msgTag "Volume Maxxed out" 
            else
                case $output_medium in
                    $speaker_ID)
                        dunstify -t 1000 -a "changeVolume" -u low -i ~/.local/darthfiles/iconss/speaker_40blue.png \
                            -h string:x-dunst-stack-tag:$msgTag -h int:value:"$int_volume" "             ${int_volume}"
                        ;;
                    $earphones_ID)
                        dunstify -t 1000 -a "changeVolume" -u low -i ~/.local/darthfiles/iconss/ear_blue_40.png \
                            -h string:x-dunst-stack-tag:$msgTag -h int:value:"$int_volume" "             ${int_volume}"
                        ;;
                    $easy_sink_ID)
                        dunstify -t 1000 -a "changeVolume" -u low -i ~/.local/darthfiles/iconss/ez.png \
        ...
        "mute")
            if [[ $mute_status == MUTED ]];then
                ...
                dunstify -t 1000 -a "changeVolume" -u low -i ~/.local/darthfiles/iconss/lol.png \
                    -h string:x-dunst-stack-tag:$msgTag "Unmuted" 
            else
                wpctl set-mute @DEFAULT_AUDIO_SINK@ 1 > /dev/null
                dunstify -t 1000 -a "changeVolume" -u low -i ~/.local/darthfiles/iconss/mute2.png \
